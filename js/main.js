var CONTRACT_ADDRESS = "TJwLq15w19XN18ZtAyoQzRnCWHT59d1TRT";
var SOFT_LAUNCH = 1602946800; 
var FULL_LAUNCH = 1603015200; 
...
function updateBTLaunchCountdown() {
  function pad(num) {
    return num > 9 ? num : '0'+num;
  };
  var
	...
	mm = mins - hours * 60,
	ss = secs - mins * 60;
	document.querySelectorAll("*[data-update='btlaunch-countdown']").forEach(function(e){
    e.innerHTML =
        dd + ' days ' +
        pad(hh) + ':' + //' hours ' +
        pad(mm) + ':' + //' minutes ' +
...
function load_contract_details(){
  window.trontrust.contract.contractInfo().call()
...
function load_account_details(){
  if(window.trontrust.address != undefined && window.trontrust.address != null){
    window.trontrust.contract.userInfo(window.trontrust.address).call()
      .then(function(response){
...
function load_account_balance(){
  if(window.trontrust.address != undefined && window.trontrust.address != null){
    window.tronWeb.trx.getAccount(window.trontrust.address)
    .then(function(response){
...
function load_account_payments(){
  if(window.trontrust.address != undefined && window.trontrust.address != null){
    window.trontrust.contract.investmentsInfo(window.trontrust.address).call()
    .then(function(response){
...
function deposit(value){
  window.trontrust.contract.deposit(window.referral).send({ callValue: value * 1000000 })
  .then(function(response){
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if((new Date()/1000) < FULL_LAUNCH){
    ...
    document.getElementById("withdraw_cd").style.display = 'block';
    updateBTLaunchCountdown();
    window.setInterval(updateBTLaunchCountdown, 1000);
  }
  if((new Date()/1000) > FULL_LAUNCH){
    document.getElementById("withdraw").style.display = 'block';
    document.getElementById("withdraw_cd").style.display = 'none';
  ...
  var load_obj = setInterval(async ()=>{
      if (window.tronWeb && window.tronWeb.defaultAddress.base58) {
        ...
        window.trontrust.address = window.tronWeb.defaultAddress.base58;
        window.trontrust.contract = await window.tronWeb.contract().at(CONTRACT_ADDRESS)
          .catch(function(err){ swal("Oh no :/", err, "error") })
        ...
        document.getElementById("your_address").innerHTML = window.trontrust.address
        document.getElementById("referral_link").value = "https://trxtrust.org/?r="+window.trontrust.address;
        document.getElementById("your_adr").innerHTML = "<i class='fa fa-lock mr-2 text-success'></i> "+window.trontrust.address.substring(0,12)+"...";
  ...
  window.setInterval(function(){
    if(window.trontrust.address != window.tronWeb.defaultAddress.base58){
      window.trontrust.address = window.tronWeb.defaultAddress.base58;
      document.getElementById("your_address").innerHTML = window.trontrust.address;
      document.getElementById("referral_link").value = "https://trxtrust.org/?r="+window.trontrust.address;
      document.getElementById("your_adr").innerHTML = "<i class='fa fa-lock mr-2 text-success'></i> "+window.trontrust.address.substring(0,12)+"...";
  ...
  document.getElementById("withdraw").addEventListener('click', async (e)=>{
	...
	if(Math.round(new Date().getTime()/1000) < FULL_LAUNCH) {
		...
		window.trontrust.contract.withdraw().send()
  ...
  //   e.preventDefault();
  //   window.trontrust.contract.reinvest().send()
